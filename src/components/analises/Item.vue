            ...
            <TabView class="border rounded-lg py-6 shadow-2xl" headerClass="bg-black" style="width: 650px; height: 350px;">
                <TabPanel header="Geral" headerClass="w-full">
                    <ul class="mt-2">
                        ...
                        <li>Estoque: {{ produto.initial_quantity }} </li>
                        <li>Peso cadastrado: {{ shipping?.billable_weight }}g</li>
                        <li>Valor Frete Pago pelo Seller: R$ {{ shipping?.list_cost }}</li>
                        <li>Modalidade: {{ produto.shipping?.logistic_type === 'fulfillment' ? 'FULL' : 'Cross Docking' }}
                        </li>
                    </ul>
                </TabPanel>
                ...
                <TabPanel header="Categoria" headerClass="w-full">
                    <ul class="mt-2">
                        ...
                        <li class="mt-2">Categoria criada em: {{ dayjs(categoryDatas.date_created).format("DD/MM/YYYY") }}
                        </li>
                    </ul>
                </TabPanel>
                <TabPanel header="Seller" headerClass="w-full">
                    <ul class="mt-2">
                        ...
                        <li class="mt-2">Reputação: {{ seller.seller_reputation.level_id }} - {{
                            seller.seller_reputation.power_seller_status }}</li>
                    </ul>
                </TabPanel>
            </TabView>
        </div>
    </div>
    <div v-else="!produto" class="flex justify-center"><i class="pi pi pi-spin pi-spinner" style="font-size: 3rem;"></i>
    </div>
</template>
...
watchEffect(async () => {
    ...
    try {
        ...
        console.log('Meu Produto', produto.value.sold_quantity)
